...
class ColorSet:
		...
		def colorset_image(self, colorset, verbose=False, x=600, y=100):
		...
		def checkDuplicate(self, dbPath, in_filename):
		...
		def appendDatabase(self, colorset, in_filename="test.png", dbPath="./../database/colorsetDB.csv"):
			dataset = colorset
...
if __name__ == '__main__':
	...
	color_test = ColorSet()
	im = color_test.openImage(in_filePath, verbose) #Open Image
	pix = im.load() # Get the Pixel Access Object
	pix_array = color_test.imageToArray(pix) #Convert Image to Array
	colorset = color_test.cluster(pix_array) #Cluster the Array to find centroids for colorset
	if arguments['-d']:
		dbPath = arguments['<dbPath>']
		color_test.appendDatabase(colorset, in_filename, dbPath)
	else:
		color_test.appendDatabase(colorset, in_filename)
	colorset_im = color_test.colorset_image(colorset, verbose) #Generate Colorset Images
